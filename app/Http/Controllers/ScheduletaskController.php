      ...
      if(auth()->user()->userrole==1){
        ...
        if(session()->has('userid')){
          if(session()->get('userid') == 'all'){
            ...
            $all = Scheduletask::where('assigned_user_id',session()->get('userid'))->get();
          }
        }else{
          $all = Scheduletask::all();
        }
          ...
          foreach($all as $note)
          {
              ...
              $new = array(
                  ...
                  'leadownername' => User::where('id',$note->assigned_user_id)->first()->name,
                  'leadownercolor' => User::where('id',$note->assigned_user_id)->first()->colortype,
                  'start' =>  $note->date,
                  'status' => $note->status,
        ...
        //SALES
         ...
         $allu = Scheduletask::where('assigned_user_id',auth()->user()->id)->get();
         ...
         $datasu = [];
          ...
          foreach($allu as $notes)
          {
              ...
              $new = array(
                  ...
                  'leadownername' => User::where('id',$notes->assigned_user_id)->first()->name,
                  'leadownercolor' => User::where('id',$notes->assigned_user_id)->first()->colortype,
                  'start' =>  $notes->date,
                  'status' => $notes->status,
                  'className' => 'bg-soft-pink'
