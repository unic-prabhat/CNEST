      ...
      <div class="master">
         ...
         <div class="list mytask">
            ...
            <div class="head">
               ...
               {{count(App\Task::where('status_id',$lead->id)->where('userAssign_id',auth()->user()->id)->get())}}
               ...
               @foreach(App\Task::where('status_id',$lead->id)->where('userAssign_id',auth()->user()->id)->get() as $all)
            ...
            @php ($tass = App\Task::where('status_id',$lead->id)->where('userAssign_id',auth()->user()->id)->get()) @endphp
            ...
            <div class="list-item {{count($tass) > 0 ? '' : 'dropify-wrapper' }}" id="project{{$key+1}}" status="{{$lead->id}}">
               ...
               <div class="chh" status-id="{{$lead->id}}" row-id="{{$val->id}}" id="dealdelete{{$val->id}}">
                  ...
                  <div class="d-flex" style="justify-content:space-between"><strong style="font-size: 12px;color: red;"></strong></p><span class="destroy float-right" data-id="{{$val->id}}" lead-id="{{$val->lead_id}}"><i class="fas fa-window-close"></i></span></div>

                  <div class="ch">

                     <a href="{{URL::to('saleslead/'.$val->lead_id)}}" target="_blank">
                     ...
                     <h5 class="mb-2" style="font-weight: 600">
                        ...
                        <span class="price float-right" style="    position: relative;
    top: -17px;">{{$val->amount}}</span>
