               ...
               <div class="chh" status-id="{{$lead->id}}" row-id="{{$val->id}}" id="dealdelete{{$val->id}}">
                  <div class="d-flex" style="justify-content:space-between">
                    <strong style="
                     ...
                     {{App\User::where('id',$val->lead->userAssign_id)->select('name')->first()->name}}

                   </strong><span class="destroy float-right" data-id="{{$val->id}}" lead-id="{{$val->lead_id}}"><i class="fas fa-window-close"></i></span>
                  </div>
                  <div class="ch">
